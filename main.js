...
const loadCoristas=(members)=>{
    ...
    const sortedMembers = members.filter(member => member !== anonymousOption).sort((a, b) => a.localeCompare(b));
...
const sendName=async (e)=>{
    ...
    const corista=coristasList.filter((member)=>member.name===choirMember$$.value)
